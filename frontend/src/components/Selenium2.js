...
const Selenium = () => {
  ...
  const handleActionChainDelete = (index, i) => {
    // const updatedActionChain = rowData[index].actionChain.filter(
    //   (_, n) => n !== i
    // );
    // setRowData([
    //   ...rowData.filter((_, n) => n !== index),
    //   { ...rowData[index], actionChain: updatedActionChain },
    // ]);
    const copiedRows = [...rowData];
    const updatedArr = copiedRows[index].actionChain?.filter((_, n) => n !== i);
    copiedRows[index].actionChain = updatedArr;
  ...
  const handleActionChainEdit = (index, i) => {
    ...
    setActiveOperationId(index);
    setActiveActionChain(rowData[index].actionChain[i]);
    setOpenSidebar(true);
  };
  ...
  const handleActionChainSubmit = (values) => {
    if (editorAction === 'ADD') {
      rowData[activeOperationId].actionChain.push(values);
    } else {
      rowData[activeOperationId].actionChain[activeActionChainId] = values;
    }
    setOpenSidebar(false);
  };
      ...
      <div
        ...
        <div className=" w-full flex flex-col md:flex-row mt-3 gap-3">
          <TextField
            ...
            style={{ flex: 5 }}
          />
          <FormControl style={{ flex: 4, height: '45px' }}>
